...
public class CleanUpStatisticsJob extends CacheBaseJob {
    ...
    //清除standard_statistics
    private static final String CLEAN_STANDARD_STATISTICS = "delete from standard_statistics where created_time < ?";
    ...
    public void action(JobExecutionContext context) {
        ...
        try {
            ...
            //清除standard_statistics(保留7天)
            calendar.setTime(new Date());
            calendar.add(Calendar.DAY_OF_MONTH, -7);
            time = calendar.getTime();
            cleanCount = jdbcTemplate.update(CLEAN_STANDARD_STATISTICS, time);
            logger.warn("clean_standard_statistics_total count={}", cleanCount);
